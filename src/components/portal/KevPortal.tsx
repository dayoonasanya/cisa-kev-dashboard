...
export default function KevPortal(){const {dataset,status,retry:retryLoad}=useKevSnapshot();const [view,setView]=useState<PortalView>(()=>{if(typeof window==="undefined")return "command";const current=new URLSearchParams(location.search).get("view");return isView(current)?current:"command"}),[filters,setFilters]=useState<DashboardFilters>(EMPTY),[menuOpen,setMenuOpen]=useState(false),[presenting,setPresenting]=useState(()=>typeof window!=="undefined"&&new URLSearchParams(location.search).get("present")==="1");
  ...
  const exitPresentation=()=>{setPresenting(false);const url=new URL(location.href);url.searchParams.delete("present");history.replaceState({},"",url)};
  return <main className={presenting?"portal-shell presentation-mode":"portal-shell"}>{presenting&&<PresentationBar onExit={exitPresentation}/>}<div className={menuOpen?"mobile-backdrop open":"mobile-backdrop"} onClick={()=>setMenuOpen(false)}/><div className={menuOpen?"nav-wrap open":"nav-wrap"}><PortalNavigation view={view} onChange={navigate} onClose={()=>setMenuOpen(false)}/></div><section className="portal-main"><PortalHeader dataset={dataset} onMenu={()=>setMenuOpen(true)} activeFilterCount={activeFilterCount} watchlistCount={analystState.watchlist.length}/><div className="portal-content">{!presenting&&<FilterBar filters={filters} vendors={vendors} years={years} onChange={(key,value)=>setFilters((current)=>({...current,[key]:value}))} onReset={()=>setFilters(EMPTY)}/>} {content}</div><footer className="portal-footer"><span>Analysis by Adedayo A. Onasanya</span><span>{filtered.length.toLocaleString()} of {records.length.toLocaleString()} records in scope</span><span>CISA KEV data · CC0 1.0</span></footer></section></main>;
}
